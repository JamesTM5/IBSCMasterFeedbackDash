...
```{r}
selectInput(inputId = "GB",label = "Group By (x axis)",choices = GBChoices)
style = "input$GB {font-weight: bold;}"

```

```{r}
selectInput(inputId = "OB",label = "Order By (x axis)",choices = c(
  "Ascending",
  "Descending",
  "Alphabetical"))
style = "input$OB {font-weight: bold;}"

```

```{r}
selectInput(inputId = "Metric",label = "Metric (y axis)",choices =
              metricChoices)
```

The data for the Belongingness survey of class `r fileList[[1]]$className` at `r clientName` is presented on this dashboard.
