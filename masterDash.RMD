...
predictorsIn <- unprocessedBelongingnessData[1:(length(unprocessedBelongingnessData)-3)]
significantPredictors <- belongingnessSignificanceTesting(predictorsIn)
...
Student - Student Overall Findings {data-navmenu="Class Name"}
=======================================================================
...
rainCloudDataList <- list()
for (i in 1:length(fileList)) {
  rainCloudDataList[[i]] <- data.frame(fileList[[i]]$nodesRainCloud)
}
rainCloudData <- dplyr::bind_rows(rainCloudDataList)
rainCloudXChoices <- rainCloudData %>% select(contains('numeric'))   
rainCloudYChoices <- rainCloudData %>% select(!contains('numeric'))   
rainCloudXChoices <- names(rainCloudXChoices)
rainCloudYChoices <- names(rainCloudYChoices)
```
...
renderPlot({
  updateRainCloudData <- 
          rainCloudData %>%
          group_by(!!! rlang::syms(input$rainCloudX))
        rainCloudXVar <- rlang::sym(input$rainCloudX)
        rainCloudYVar <- rlang::sym(input$rainCloudY)
ggplot(updateRainCloudData, aes(!!rainCloudXVar, !!rainCloudYVar, fill = !!rainCloudYVar)) + 
  ggdist::stat_halfeye(adjust = .5, .width = 0, point_colour = NA) + 
  ggdist::stat_dots(side = "left", dotsize = .1, justification = 1.05, binwidth = .25)
})
...
renderPlot({networkHealth + PfIPlot})
```