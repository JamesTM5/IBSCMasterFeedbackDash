class Parliament{
 ...
 setColor( fillFunction , opacityFunction ){
 ...
 .attr("x", d=>this.fixNaN(d.x))
 .attr("y", d=>this.fixNaN(d.y))
 .attr("opacity", d=>this.validPositionOpacity(d) * this.opacityFunction(d) )
 .attr("width", config.squareWidth)
 .attr("height", config.squareHeight),
 exit => exit
 .remove());
 ...
 validPositionOpacity(d){
 if ( Number.isNaN(d.x) || Number.isNaN(d.y) || d.x===undefined || d.y===undefined ){
 return 0;
 }else{
 return 1;
 }
...
class ParliamentControl extends Parliament {
 ...
 function genderOpacity(d){
 if (d.gender=="m") return 1;
 if (d.gender=="f") return 1;
 if (d.gender=="n") return 1;
 return 0;
 }
 ...
 this.pbs = new PlaybackSystem( this.control.target , ()=>this.store() , (s)=>this.restore(s) );
 ...
 function setF( target ){
 elem.node().value = target;
 }
 ...
 if ( sorter && this.data ){
 this.setColor( sorter.color , sorter.opacity );
 this.transition(100,25);
 }
 }
 store(){
 const details = [ this.control.layoutDropdown ,this.control.orderA,this.control.orderB,this.control.colorDropdown];
 return details.map( d=>d.get().key ).join(":_:");
 }
 restore( dataStr ){
 const details = dataStr.split(":_:");
 this.control.layoutDropdown.set(details[0]);
 this.control.orderA.set(details[1]);
 this.control.orderB.set(details[2]);
 this.control.colorDropdown.set(details[3]);
 this.colorChanged();
 this.layoutChanged();
 }
}
...
class PlaybackSystem{
 constructor(div_element , getCurrentF , onChangeF ){
 this.states = [];
 this.currentIndex = 0;
 this.getCurrentF = getCurrentF;
 this.onChangeF = onChangeF;
 this.controlsDiv = div_element.append("span").attr("class","savedStatesControls");
 this.stateDiv = div_element.append("span").attr("class","savedStates");
 ...
 saveState(){
 const newState = this.getCurrentF();
 this.states.push( newState );
 this.update();
 }
 update(){
 const currentIndex = this.currentIndex ;
 function buttonTextFunction(d,i){
 if ( i == currentIndex ){
 return "O";
 }else{
 return ".";
 ...
 this.stateDiv.selectAll("button")
 .data(this.states) 
 .join(
 ...
 .on("click",(event,data)=>{
 this.onChangeF(data); 
 this.currentIndex=this.states.indexOf(data);
 this.update();
 } )
 .text( buttonTextFunction ),
 update => update
 .text( buttonTextFunction ),
 exit => exit
 .remove());
 ...
 this.onChangeF( this.states[this.currentIndex] );
 }
 this.update();
 }
}

On Fri, 26 Aug 2022 at 13:26, Chris Major <chris.j.major@gmail.com> wrote:
You should be ok to replace the parliament.js with:
